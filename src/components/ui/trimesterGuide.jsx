export default function TrimesterGuide() {
    ...
    return (
        <div>
            <section className="w-full bg-warm-cream py-14 px-4 mb-70">
      <div className="max-w-7xl mx-auto">

        <div className="text-center mb-10">
          <h2 className="text-4xl sm:text-5xl font-playfair font-bold text-saddle-brown mb-3">
            Trimester Guide
          </h2>
          <p className="text-md sm:text-xl text-modern-earthy">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-50">
          {trimesters.map((t) => (
            <div key={t.number} className="bg-simple-white rounded-2xl border border-gray-200 p-6 flex flex-col gap-4">

              <span className="text-5xl font-bold text-classy-red/50">{t.number}</span>

              <div>
                <h3 className="text-xl font-playfair font-bold text-saddle-brown">
                  {t.title}
                </h3>
                <p className="text-xl text-classy-red font-medium mt-0.5">{t.weeks}</p>
              </div>

              <ul className="flex flex-col gap-2">
                {t.points.map((point, i) => (
                  <li key={i} className="flex items-start gap-2 text-xl text-modern-earthy">
                    <span className="text-classy-red mt-0.5 shrink-0">✓</span>
                    {point}
                  </li>
                ))}
              </ul>
